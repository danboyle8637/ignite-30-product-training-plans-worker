CREATE OR REPLACE PROCEDURE training_plans.toggle_is_busy_goal_complete(
...
BEGIN
    WITH updated_daily_points AS (
        SELECT
                daily_points + CASE
                                   WHEN is_busy_goal_complete = FALSE AND is_primary_goal_complete = FALSE THEN 2
                                   WHEN is_busy_goal_complete = FALSE AND is_primary_goal_complete = TRUE THEN -1
                                   WHEN is_busy_goal_complete = TRUE AND is_primary_goal_complete = FALSE THEN -2
                                   ELSE 0
                ...
                CASE
                    ...
                    ELSE 0
                    END AS points_diff,
                is_primary_goal_complete as initial_primary_goal_state
        ...
        SELECT
            NOT is_busy_goal_complete as new_is_busy_goal_complete
        FROM training_plans.training_plan_day_stats
        WHERE user_id = arg_user_id
          AND training_plan_stats_id = arg_training_plan_stats_id
          AND training_plan_day = arg_training_plan_day
        ...
        UPDATE training_plans.training_plan_day_stats
            SET
                ...
                is_busy_goal_complete = (SELECT new_is_busy_goal_complete FROM toggle_busy_goal),
                is_primary_goal_complete = FALSE
            WHERE user_id = arg_user_id
                AND training_plan_stats_id = arg_training_plan_stats_id
                AND training_plan_day = arg_training_plan_day
            RETURNING
        ...
        INSERT INTO training_plans.training_plan_day_stats
            (user_id, program_id, training_plan_stats_id, training_plan_day, day_date, daily_points, is_busy_goal_complete)
            SELECT arg_user_id, arg_program_id, arg_training_plan_stats_id, arg_training_plan_day, arg_day_date, 2, TRUE
        ...
        INSERT INTO training_plans.previous_days_missed_streak
            ...
            WHERE COALESCE((SELECT is_busy_goal_complete FROM update_existing_day_stats), (SELECT is_busy_goal_complete FROM insert_if_not_exists_day_stats)) = TRUE
        ...
        WHERE (SELECT days_missed_streak FROM current_stats) = 0
          ...
          AND training_plan_stats_id = arg_training_plan_stats_id
          AND day_date = arg_day_date
    ),save_current_primary_goal_streak AS (
        INSERT INTO training_plans.previous_primary_goal_streak
            (user_id, program_id, training_plan_stats_id, day_date, previous_primary_goal_streak)
            SELECT
                arg_user_id,
                arg_program_id,
                arg_training_plan_stats_id,
                arg_day_date,
                (SELECT primary_goal_streak FROM current_stats)
            WHERE COALESCE(
                          (SELECT is_busy_goal_complete FROM update_existing_day_stats),
                          (SELECT is_busy_goal_complete FROM insert_if_not_exists_day_stats)
                      ) = TRUE
              AND (SELECT primary_goal_streak FROM current_stats) > 0
    ), new_training_plan_stats_data AS (
        SELECT
            CASE
                WHEN COALESCE((SELECT is_busy_goal_complete FROM update_existing_day_stats), (SELECT is_busy_goal_complete FROM insert_if_not_exists_day_stats)) = TRUE
                    THEN CASE
                             WHEN (SELECT is_complete_day FROM current_stats) = TRUE AND (SELECT initial_primary_goal_state FROM updated_daily_points) = FALSE
                                 THEN total_points + COALESCE((SELECT points_diff FROM updated_daily_points), 2) + 2
                             ELSE total_points + COALESCE((SELECT points_diff FROM updated_daily_points), 2)
                    END
                ELSE CASE
            ...
            CASE
                WHEN (SELECT is_complete_day FROM current_stats) = TRUE
                    AND COALESCE((SELECT is_busy_goal_complete FROM update_existing_day_stats), (SELECT is_busy_goal_complete FROM insert_if_not_exists_day_stats)) = TRUE
                    AND (SELECT initial_primary_goal_state FROM updated_daily_points) = FALSE
