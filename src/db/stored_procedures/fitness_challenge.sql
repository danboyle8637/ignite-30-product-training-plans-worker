CREATE OR REPLACE PROCEDURE training_plans.toggle_is_fitness_challenge_complete(
    ...
    WITH update_existing_day_stats AS (
        UPDATE training_plans.training_plan_day_stats
            SET daily_points = daily_points + CASE
                                                  WHEN is_fitness_challenge_complete = FALSE THEN 1
                                                  ELSE -1
                END,
                is_fitness_challenge_complete = NOT is_fitness_challenge_complete
        ...
        INSERT INTO training_plans.training_plan_day_stats
            (user_id, program_id, training_plan_stats_id, training_plan_day, day_date, daily_points, is_fitness_challenge_complete)
        ...
        INSERT INTO training_plans.previous_days_missed_streak
            ...
            WHERE COALESCE((SELECT is_fitness_challenge_complete FROM update_existing_day_stats), (SELECT is_fitness_challenge_complete FROM insert_if_not_exists_day_stats)) = TRUE
        ...
        SELECT
            ...
            tpd.is_nutrition_challenge_complete,
            tpd.is_nutrition_challenge_complete
                AND COALESCE((SELECT is_fitness_challenge_complete FROM insert_if_not_exists_day_stats),
                             (SELECT is_fitness_challenge_complete FROM update_existing_day_stats)) AS is_complete_day
        ...
        SELECT CASE
                   WHEN COALESCE((SELECT is_fitness_challenge_complete FROM insert_if_not_exists_day_stats),
                                 (SELECT is_fitness_challenge_complete FROM update_existing_day_stats)) = TRUE
                       THEN CASE
                                WHEN (SELECT is_nutrition_challenge_complete AND (is_primary_goal_complete OR is_busy_goal_complete) FROM current_stats) = TRUE
                                    THEN total_points + 3
                                ELSE total_points + 1
                       END
                   ELSE CASE
               ...
               CASE
                   WHEN COALESCE((SELECT is_fitness_challenge_complete FROM update_existing_day_stats), (SELECT is_fitness_challenge_complete FROM insert_if_not_exists_day_stats)) = TRUE
                       OR (
                   ...
                   WHEN arg_current_days_missed = 0
                       AND (
                                ...
                                AND (SELECT is_primary_goal_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_busy_goal_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_fitness_challenge_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_nutrition_challenge_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_motivation_challenge_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_coaching_class_complete FROM update_existing_day_stats) = FALSE
                                AND (SELECT is_step_challenge_complete FROM update_existing_day_stats) = FALSE
                            )
               ...
               CASE
                   ...
                   ELSE CASE
                            WHEN is_nutrition_challenge_complete = TRUE
                                AND (COALESCE((SELECT is_primary_goal_complete FROM update_existing_day_stats),
                                              (SELECT is_primary_goal_complete FROM insert_if_not_exists_day_stats)) = TRUE
                                    OR COALESCE((SELECT is_busy_goal_complete FROM update_existing_day_stats),
                                                (SELECT is_busy_goal_complete FROM insert_if_not_exists_day_stats)) = TRUE)
