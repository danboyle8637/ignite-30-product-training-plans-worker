CREATE OR REPLACE PROCEDURE training_plans.toggle_is_motivation_challenge_complete(
    ...
    WITH update_existing_day_stats AS (
        UPDATE training_plans.training_plan_day_stats
            SET daily_points = daily_points + CASE
                                                  WHEN is_motivation_challenge_complete = FALSE THEN 1
                                                  ELSE -1
                END,
                is_motivation_challenge_complete = NOT is_motivation_challenge_complete
        ...
        INSERT INTO training_plans.training_plan_day_stats
            (user_id, program_id, training_plan_stats_id, training_plan_day, day_date, daily_points, is_motivation_challenge_complete)
        ...
        INSERT INTO training_plans.previous_days_missed_streak
            ...
            WHERE COALESCE((SELECT is_motivation_challenge_complete FROM update_existing_day_stats), (SELECT is_motivation_challenge_complete FROM insert_if_not_exists_day_stats)) = TRUE
        ...
        SELECT CASE
                   WHEN COALESCE((SELECT is_motivation_challenge_complete FROM insert_if_not_exists_day_stats),
                                 (SELECT is_motivation_challenge_complete FROM update_existing_day_stats)) = TRUE
                       THEN total_points + 1
                   ELSE total_points - 1
                   END AS new_total_points,
               CASE
                   WHEN COALESCE((SELECT is_motivation_challenge_complete FROM update_existing_day_stats), (SELECT is_motivation_challenge_complete FROM insert_if_not_exists_day_stats)) = TRUE
                       OR (
