CREATE OR REPLACE PROCEDURE training_plans.toggle_is_coaching_class_complete(
    ...
    WITH update_existing AS (
        UPDATE training_plans.training_plan_day_stats
            SET daily_points = daily_points + CASE
                                                  WHEN is_coaching_class_complete = FALSE THEN 1
                                                  ELSE -1
                END,
                is_coaching_class_complete = NOT is_coaching_class_complete
        ...
        INSERT INTO training_plans.training_plan_day_stats
            (user_id, program_id, training_plan_stats_id, training_plan_day, day_date, daily_points, is_coaching_class_complete)
        ...
        INSERT INTO training_plans.previous_days_missed_streak
            ...
            WHERE COALESCE((SELECT is_coaching_class_complete FROM update_existing), (SELECT is_coaching_class_complete FROM insert_if_not_exists)) = TRUE
              ...
              AND COALESCE((SELECT is_step_challenge_complete FROM update_existing), (SELECT is_step_challenge_complete FROM insert_if_not_exists)) = FALSE
        ...
        SELECT CASE
                   WHEN COALESCE((SELECT is_coaching_class_complete FROM insert_if_not_exists),
                                 (SELECT is_coaching_class_complete FROM update_existing)) = TRUE
                       THEN total_points + 1
                   ELSE total_points - 1
                   END AS new_total_points,
               CASE
                   WHEN COALESCE((SELECT is_coaching_class_complete FROM update_existing), (SELECT is_coaching_class_complete FROM insert_if_not_exists)) = TRUE
                       OR (
                                ...
                                OR  COALESCE((SELECT is_step_challenge_complete FROM update_existing), (SELECT is_step_challenge_complete FROM insert_if_not_exists)) = TRUE
                            )
                       THEN 0
