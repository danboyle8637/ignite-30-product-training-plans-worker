...
export async function saveTrainingPlan(ctx: Context): Promise<Response> {
	...
	try {
		await admin.saveTrainingPlan(param);
		const response = new Response("Trainig Plan Saved", { status: 200 });
		return response;
	} catch (error) {
