...
import type { GetPserCoachingClassesData } from "../../types/sanity";
import type { PserCoachingClassesResBody } from "../../types/responses";

export async function getPserCoachingClasses(ctx: Context): Promise<Response> {
	const req = ctx.req.raw;
	const pathname = new URL(req.url).pathname;
	const headers = req.headers;
	const contentType = headers.get("Content-Type");
	const authorization = headers.get("Authorization") || "";
	const paramsOrder = ctx.req.param("order") as string;
	const env: Env = ctx.env;

	// LOGGING
	const endpoint = `/get-pser-coaching-classes-data`;
	const handlerFunction: HandlerFunction = "getPserCoachingClasses";

	if (contentType !== JSON_CONTENT_TYPE || paramsOrder === undefined) {
	...
	try {
		const coachingDataOrder = Number(paramsOrder);
		const pserCoachingClassesData: GetPserCoachingClassesData = await programs.getPserCoachingClassesData(coachingDataOrder);
		const coachingClasses = pserCoachingClassesData.pserCoachingClassesData.videos;

		const resBody: PserCoachingClassesResBody = {
